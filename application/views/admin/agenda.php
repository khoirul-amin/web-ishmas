        ...
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <form id="formInformasi" enctype="multipart/form-data" class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="inputJudul">Waktu</label>
                        <input type="text" name="waktu" class="form-control" id="waktu" placeholder="..." required>
                    </div>
                    <div class="form-group">
                        <label for="inputJudul">Tempat</label>
                        <input type="text" name="tempat" class="form-control" id="tempat" placeholder="..." required>
                    </div>
                    <div class="form-group">
  ...
  <script>
        ...
        function getData(){
            $('#example1').DataTable({
                ...
                ajax:{
                    url: "<?php echo base_url("Admin/Agenda/get_datatables");?>",
                    type: "POST",
                    cache: false,
                },
            });
        ...
        function updateInformasi(id){
            $.ajax({
                type: "GET",
                url: `/Admin/Agenda/getByID/${id}`,
                cache: false,
                success: function(data){
                    const dataOk = JSON.parse(data)
                    $('#id').val(dataOk.id)
                    $('#judul').val(dataOk.judul)
                    $('#tanggal').val(dataOk.tanggal)
                    $('#waktu').val(dataOk.waktu)
                    $('#tempat').val(dataOk.tempat)
